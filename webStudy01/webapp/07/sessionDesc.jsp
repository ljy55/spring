...
<pre>
	세션 이란?
		...
		시간(web) : 클라이언트가 서버의 어플리케이션을 사용하고 있는 동안을 한 세션으로 정의함.
					최초의 요청(세션 생성의 대상은 브라우저) ~~~ 종료 조건      (1. timeout 이내에 새로운 요청이 없을 때.)
																(2. 브라우저의 종료)
																(3. 더이상 쿠키(JSSESSIONID) 전송이 없을 때)
																(4. 직접 로그아웃을 한 경우)
	session(HttpSession) : 한 세션 내에서 발생한 클라이언트의 모든 정보를 캡술화한 객체.
	세션 ID : <%=session.getId() %>
	...
	트래킹 모드
	1. COOKIE : ex) JESSIONID와 같은 세션 아이디를 식별할 수 있는 쿠키를 C/S 사이에서 주고 받는 방법.
	2. SSL : secure layer 를 통해 세션 아이디를 주고 받는 방법. //TLS/SSL// 
			  전송 계층을 보안 처리 하여 데이터를 보호하는 방법의 일종.
