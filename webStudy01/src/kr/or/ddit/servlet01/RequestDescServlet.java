...
/**
 * Http 의 request 명세
 * 1) request Line : Protocol/version, URI(수신자), Method(요청의 목적, 방법)
 ...
 * 		- OPTION : prefligt 요청에 사용, 서버의 메소드 지원여부를 확인할 용도.
 * 		- TRACE : 서버를 디버깅할 용도.
 * 2) request Header : 클라이언트의 해당 클라이언트의 사용 시스템에 대한 부가정도(meta data)
...
public class RequestDescServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.setCharacterEncoding("UTF-8"); //server.xml에 useBodyEncodingForURI="true"
		// 요청 파라미터가 어떻게 전달되길래?!
		// 파라미터는 특수문자를 '%E3'과 같은 형태로 인코딩이 되어 전달됨. - REF-2396 규칙에 따라 정의됨.
