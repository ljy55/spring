...
public class ServerExplorerServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		/*
		 * String ctxName = req.getParameter("ctxName"); System.out.println(ctxName);
		 * ServletContext application = this.application;
		 * if(StringUtils.isNotBlank(ctxName)) { application =
		 * getServletContext().getContext(ctxName); } if(application==null) {
		 * resp.sendError(404, ctxName+"컨텍스트는 존재하지 않습니다."); return; }
		 */
		...
		if (accept.contains("json")) {
			...
			resp.setContentType("application/json;charset=UTF-8");
			ObjectMapper mapper = new ObjectMapper();
			String json = mapper.writeValueAsString(wrappers); // read - 언마샬링
			try (PrintWriter out = resp.getWriter();) {
				// 직렬화(serialize)
				out.print(json);
			}
